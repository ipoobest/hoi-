...
export default {
  ...
  created() {
    this.initial();
    this.$EventBus.$on("onTotalStockRefresh", this.initial);
  },
  beforeDestroy() {
    this.$EventBus.$off("onTotalStockRefresh", this.initial);
  },
  methods: {
    initial() {
      getAvailableLotto("?status=OPEN&active=true&type_in=STOCK").then(
        items => {
          this.items = items;
        }
      );
