      ...
      <template v-for="(item, index) in items">
        ...
        >
          <v-hover v-slot:default="{ hover }">
            ...
            >
              <v-list-item
                :to="item.type === 'YEEKEE' ? `/report/lotto/${item._id}` : ''"
              >
                <v-list-item-content>
                  <v-row align="stretch">
                    <v-col
  ...
  methods: {
    initial() {
      ...
      const endedAt = this.$Moment(this.inqueryDate, "YYYY-MM-DD")
        ...
        .format();
      getAvailableLotto(
        `?status=OPEN&active=true&type_in=LOTTO&type_in=YEEKEE&resultedAt_gte=${encodeURIComponent(
          startedAt
