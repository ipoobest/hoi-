...
import { columnDefs } from "@/views/report/grid/WinLossColumnDefs.js";
...
export default {
  ...
  methods: {
    ...
    updateBreadcrumbs(locale) {
      ...
      if (this.productLotto) {
        this.breadcrumbsItems.push({
          ...
          exact: true,
          to: "/report/overall/round/" + this.productLotto._id
        });
      }
      if (this.lotto) {
        this.breadcrumbsItems.push({
          text: this.lotto[getLangNameKeyFromLocale(locale)],
          disabled: true
        });
      }
    },
    async initial() {
      ...
      this.productLotto = await getProductLottoById(this.lotto.productLottoId);
