...
import { getGridUsersLoginSuccess } from "@/api/user";

import MemberId from "@/views/member/grid/MemberId";
import { columnDefs } from "@/views/member/grid/PlayerSucceedLogColumnDefs.js";
Object.freeze(columnDefs);

import AgGridHeaderI18n from "@/components/AgGridHeaderI18n";

export default {
  name: "PlayerSucceedLog",
  components: {
    AgGridVue,
    /* eslint-disable */
    AgGridHeaderI18n,
  ...
  created() {
    ...
    this.worker = setInterval(() => {
      if (this.doRefreshGrid) {
        this.doRefreshGrid = false;
        this.onPlayerSucceedLogRefresh();
      }
    }, 5000);
    this.$EventBus.$on("onWebSocketUserLogin", this.onWebSocketUserLogin);
    this.$EventBus.$on(
      "onPlayerSucceedLogRefresh",
      this.onPlayerSucceedLogRefresh
    );
  },
  beforeDestroy() {
    ...
    this.$EventBus.$off("onWebSocketUserLogin", this.onWebSocketUserLogin);
    this.$EventBus.$off(
      "onPlayerSucceedLogRefresh",
      this.onPlayerSucceedLogRefresh
    );
