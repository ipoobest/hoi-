        ...
        <v-col v-for="obj in props.items" :key="obj.key" class="py-0">
          <v-card class="my-0">
            ...
            <v-list dense>
              <template v-for="data in obj.data">
                ...
                >
                  ...
                  <v-list-item-content
                    ...
                    class="align-end"
                  >
                    <a v-if="data.value" :href="`tel:${data.value}`">{{
                      data.value
                    }}</a>
                    <span v-else>-</span>
                  </v-list-item-content>
                  ...
                  >
                    <router-link
                      v-if="data.value"
                      tag="a"
                      :to="`/member/${user.parentUserId}`"
                      >{{ data.value }}</router-link
                    >
                    <span v-else>-</span>
                  </v-list-item-content>
                  <v-list-item-content
                    v-else-if="data.key === 'type'"
                    class="align-end"
                  >
                    <template
                      v-if="
                        user && ['Modulator', 'Partner'].includes(me.role.name)
                      "
                    >
                      <v-select
                        v-model="userType"
                    ...
                    <template v-else>
                      <span>{{ $t(data.value.toLowerCase()) }}</span>
                    </template>
                  </v-list-item-content>
              ...
              <v-list-item>
                <v-list-item-content>
                  <strong>{{ $t("verify_phone") }}</strong>
                </v-list-item-content>
                <v-list-item-content class="align-end">
                  <v-select
                    v-model="phoneVerified"
                    :items="phoneVerifiedItems"
