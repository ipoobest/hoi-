...
<script>
import { incomeTransactionTypes } from "@/utils/IncomeTransactionTypeFormatter";
export default {
  name: "IncomeTransactionTypeDropdown",
  props: {
    value: {
      type: Object,
      required: true
  ...
  methods: {
    onLocaleChanged() {
      this.items = [];
      this.$_.forOwn(incomeTransactionTypes, (value, key) => {
        this.items.push({
          label: this.$t(value),
          value: key
        });
