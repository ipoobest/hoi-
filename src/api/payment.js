...
export const getPayment = async (
  ...
  const response = await axios.get(
    `/payment-transactions?_start=${startRow}&_limit=${
      endRow - startRow
    }${sortParams}` +
      req +
      filterParams
...
export const getPaymentById = async id => {
  const response = await axios.get(`/payment-transactions/${id}`);
  return response.data;
};

export const updatePaymentById = async (id, req) => {
  const response = await axios.put(`/payment-transactions/${id}`, req);
  return response.data;
};

export const addPayment = async req => {
  const response = await axios.post("/payment-transactions", req);
  return response.data;
};
